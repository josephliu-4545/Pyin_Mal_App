...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _next() {
    ...
    if (widget.isCard) {
      if (_otpCtrl.text.trim().length < 4) {
        setState(() => _error = 'payment.err_otp'.tr());
        return;
      }
      _startProcessing(1);
      return;
    ...
    if (_step == 0) {
      ...
      if (digits.length < 7) {
        setState(() =>
            _error = 'payment.err_account'.tr(namedArgs: {'wallet': widget.label}));
        return;
      }
      setState(() => _step = 1);
    } else if (_step == 1) {
      if (_pinCtrl.text.trim().length < 6) {
        setState(() => _error = 'payment.err_pin'.tr());
        return;
      }
      _startProcessing(2);
    }
  ...
  Widget _phoneStep(bool isDark, Color ink) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('payment.wallet_title'.tr(namedArgs: {'wallet': widget.label}),
            style: GoogleFonts.rufina(
                fontSize: 22, fontWeight: FontWeight.bold, color: ink)),
        const SizedBox(height: 6),
        Text('payment.wallet_desc'.tr(namedArgs: {'wallet': widget.label}),
            style: GoogleFonts.outfit(
                fontSize: 13,
                height: 1.4,
  ...
  Widget _pinStep(bool isDark, Color ink) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('payment.pin_title'.tr(namedArgs: {'wallet': widget.label}),
            style: GoogleFonts.rufina(
                fontSize: 22, fontWeight: FontWeight.bold, color: ink)),
        const SizedBox(height: 6),
        Text('payment.pin_desc'.tr(),
            style: GoogleFonts.outfit(
                fontSize: 13,
                height: 1.4,
                color: isDark ? AppColors.paleText : AppColors.inkGrey)),
        ...
        Row(
          children: [
            ...
            Expanded(
              child: Text(
                  'payment.pin_secure'.tr(namedArgs: {'brand': 'Ta Chat Nhate'}),
                  style: GoogleFonts.outfit(
                      fontSize: 11.5,
                      color: isDark ? AppColors.paleText : AppColors.inkGrey)),
            ),
  ...
  Widget _otpStep(bool isDark, Color ink) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('payment.otp_title'.tr(),
            style: GoogleFonts.rufina(
                fontSize: 22, fontWeight: FontWeight.bold, color: ink)),
        const SizedBox(height: 6),
        Text('payment.otp_desc'.tr(),
            style: GoogleFonts.outfit(
                fontSize: 13,
                height: 1.4,
                color: isDark ? AppColors.paleText : AppColors.inkGrey)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text('payment.processing_hint'.tr(),
  ...
  Widget _bottomBar(bool isDark) {
    final String label;
    if (widget.isCard) {
      label = 'payment.verify_pay'.tr(namedArgs: {'amount': widget.amountLabel});
    } else if (_step == 0) {
      label = 'payment.continue'.tr();
    } else {
      label = 'payment.pay'.tr(namedArgs: {'amount': widget.amountLabel});
    }
    return Padding(
